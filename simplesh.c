...
// Cerrar ficheros
		...
		char* home=getenv("HOME");	
		char pathFich[TAM_PATH];
		snprintf(pathFich,TAM_PATH,"%s/.tee.log",home);
		...
		fd[0]=open(pathFich,O_RDWR|O_CREAT|O_APPEND,S_IRUSR|S_IWUSR);	
			if (fd[0]==0)
			{
				perror("open");
				exit(EXIT_FAILURE);
		...
		char tiempo[TAM_PATH];
		strftime(tiempo,80,"%Y-%m-%d  %X:", info);

		int pid =(int)getpid();
		int euid=(int)getuid();
		int cx=snprintf(buffer, TAM_PATH, "%sPID %d:EUID %d:%d byte(s):%d file(s)\n", tiempo,pid, euid,bytesLeidosTotales,ficherosEscritos);
		if (cx<0 || cx>=TAM_PATH) {
			fprintf(stderr,"No se escribió correctamente en .tee.log");
		}
		bytesEscritos=write(fd[0], buffer,cx);
		if (bytesEscritos==-1)
		{
			perror("write");
			exit(EXIT_FAILURE);
		}
		int bytesEscritosTotales=bytesEscritos;
		while (bytesEscritosTotales<cx)
		{
			bytesEscritos=write(fd[0], buffer+bytesEscritosTotales,cx-bytesEscritosTotales);
			if (bytesEscritos==-1)
			{
				perror("write");
				exit(EXIT_FAILURE);
...
{	
	...
	{
		int option_index=0;
		variables=getopt(argc, argv, "hbt:v");
		if(variables==-1) 
			break;
		switch(variables)
		{
